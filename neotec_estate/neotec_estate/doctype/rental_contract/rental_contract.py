...
from neotec_estate.helpers import get_status, get_debit_to, set_invoice_created
...
def _generate_invoices_now(renting):
    ...
    rental_item = frappe.db.get_single_value(
        "Neotec Estate Settings", "rental_item"
    )
    submit_si = frappe.db.get_single_value("Neotec Estate Settings", "submit_si")
...
def _set_property_as_vacant(renting):
    retain_rental_on_cancel = frappe.db.get_single_value(
        "Neotec Estate Settings", "retain_rental_on_cancel"
    )
...
def _get_invoice_on_start_date():
    return frappe.db.get_single_value(
        "Neotec Estate Settings", "invoice_on_start_date"
    )
