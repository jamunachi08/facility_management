...
{% include 'neotec_estate/neotec_estate/doctype/property_maintenance/property_maintenance_dialog.js' %}
...
function _set_custom_buttons(frm) {
    ...
    if ((frm.doc.materials || []).some(row => row.material_status in {'Bill': 1, 'Self Consumption': 1} && !row.invoiced)) {
        frm.add_custom_button(__('Bill Materials'), async function() {
            await frappe.call({
                method: 'neotec_estate.fm_pro.maintenance_material.bill_materials',
                args: { property_maintenance: frm.doc.name },
            });
            frm.reload_doc();
        }, __('Actions'));
    }
